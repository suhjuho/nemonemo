...
#### 렌더링 최적화: 카메라 위치에 따른 상태값 변경

게임 플레이 중 사용자는 퍼즐을 회전시킬 수 있습니다. 이때 카메라의 위치가 변경되는데 카메라 위치에 따라 변경 시켜야 하는 상태값들이 있습니다.
초기에는 react three drei 라이브러리에서 제공하는 카메라 제어 컴포넌트인 orbitControls를 사용하여 카메라에 접근하였습니다.
orbitControls는 내부적으로 카메라 위치를 업데이트하면서 프레임 사이에 카메라의 위치를 집어넣는 방식을 사용하여 직접적으로 카메라 위치에 접근하는 것보다 지연을 초래하는 문제가 있었습니다.
이를 해결하기 위해 r3f라이브러리의 useThree 훅을 이용하였습니다. 이 방법으로 직접 카메라에 접근하여 위치를 가져오고 useFrame으로 매 프레임마다 상태값을 확인해주었습니다.

카메라의 위치를 변경하는 컴포넌트와 카메라 위치에 접근하여 상태값을 변경하는 로직을 구분하여 퍼즐 회전시 렌더링 성능을 높일 수 있었습니다.

#### 퍼즐 창작 GUI

새로운 퍼즐을 생성할 때 필요한 정보로 퍼즐의 사이즈(가로, 높이, 세로), 정답 큐브들의 좌표 위치, 색상 등이 있습니다. 매번 새로 퍼즐을 생성할 때마다 해당 값들을 직접 타이핑하여 DB에 추가해주었습니다. 하지만 퍼즐의 크기가 커질수록 고려해야할 데이터 양이 많아져 퍼즐을 생성하는데 어려움이 있었습니다.
직관적이고 간편한 방법으로 퍼즐을 제작할 수 있는 GUI를 개발하였습니다. 퍼즐의 좌표값을 일일이 계산할 필요가 없어졌고, 미리 완성된 퍼즐의 모습까지 확인할 수 있게 되었습니다. 플레이어도 본인만의 퍼즐을 쉽게 창작할 수 있도록 하였습니다.
